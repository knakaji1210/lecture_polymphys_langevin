# Animation of 1d Langevin Particle
...
import LangevinFunc_1d_v0 as lvf
...
t_array, x_array, vx_array, tmax = lvf.Langevin_1d_dt01(m, zeta, kBT, N)

d_array = lvf.distFromOrigin_1d(x_array)
d2_array = lvf.dist2FromOrigin_1d(x_array)
...
fig_title1 = "1-dimensional Langevin Particle ({0} steps, d$t$ = 0.1 ms)".format(N)
fig_title2 = "$d^{{2}}$ vs $t$"
...
ax1 = fig.add_subplot(121, title=fig_title1, xlabel='$t$ [ms]', ylabel='$X$ [um]',
                     xlim=[0, t_array[-1]], ylim=[-plot_lim , plot_lim])
...
LangevinParticle_1d = amp.blocks.Line(t_array_steps, x_array_steps, ax=ax1, ls='-', marker="o", markersize=2, color='blue')
...
anim = amp.Animation([LangevinParticle_1d, dist2Origin], timeline)
anim.controls()

savefile = "./gif/LangevinParticle_1d_{0}steps_with_MSD_ani".format(N)
