# Statistics of 1d Langevin Particle
...
for i in range (M):
    t_array, x_array, vx_array, tmax = lvf.Langevin_1d_dt001(m, zeta, kBT, N)

    d_array = lvf.distFromOrigin_1d(x_array)
    d2_array = lvf.dist2FromOrigin_1d(x_array)
    vACF_array = lvf.velocityACF_1d(vx_array)
...
slope = param[0]
diffConst = slope / 2.0     # 1次元なので<d^2> = 2Dt
sect = param[1]
err_diffConst = np.sqrt(cov[0][0]) / 2.0
...
savefile = "./png/LangevinParticle_1d_MSD_ACF_N{0}_M{1}".format(N, M)
