...
# v1では速度の自己相関関数もやってみた・・・間違った計算をしていたので、v1はlegacyへ

import numpy as np
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit
import animatplot as amp
import LangevinFunc_v0 as lvf
...
for i in range (M):
    ...
    veloSCF_array = lvf.velocitySCF(vx_array, vy_array) # 間違った自己相関関数の定義で計算しているのでもう使わない

    t_list_repeat.append(t_array)
    d2_list_repeat.append(d2_array)
    veloSCF_list_repeat.append(veloSCF_array)
...
veloSCF_list = lvf.calcMean(veloSCF_list_repeat, N, M)
...
param, cov = curve_fit(lvf.expFit, t_mean_list, veloSCF_list)
ampl = param[0]
tau = param[1]
base = param[2]
...
veloSCF_fit_list = [ lvf.expFit(tim, ampl, tau, base) for tim in t_mean_list ]
...
ax2.scatter(t_mean_list, veloSCF_list, marker="o", color="green")
ax2.plot(t_mean_list, veloSCF_fit_list, lw=1, color='black')
...
savefile = "./png/LangevinParticle_D&SCF_N{0}_M{1}".format(N, M)
